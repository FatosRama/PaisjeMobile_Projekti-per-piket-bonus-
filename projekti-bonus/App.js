...
import { auth, GoogleAuthProvider, FacebookAuthProvider, GithubAuthProvider, OAuthProvider, signInWithCredential } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    if (gResponse?.type === 'success') {
      ...
      signInWithCredential(auth, credential).then(u => console.log('Google user:', u.user));
    }
  }, [gResponse]);

  // ---------- Facebook ----------
  const [fbRequest, fbResponse, fbPromptAsync] = Facebook.useAuthRequest({ clientId: facebookAppId });

  useEffect(() => {
    if (fbResponse?.type === 'success') {
      const { authentication } = fbResponse;
      const credential = FacebookAuthProvider.credential(authentication.accessToken);
      signInWithCredential(auth, credential).then(u => console.log('Facebook user:', u.user));
    }
  }, [fbResponse]);
  ...
  useEffect(() => {
    if (ghResponse?.type === 'success') {
      ...
      const getToken = async () => {
        ...
        signInWithCredential(auth, credential).then(u => console.log('GitHub user:', u.user));
      };
      getToken();
    }
  }, [ghResponse]);
  ...
  useEffect(() => {
    if (msResponse?.type === 'success') {
      ...
      const getToken = async () => {
        ...
        signInWithCredential(auth, credential).then(u => console.log('Microsoft user:', u.user));
      };
      getToken();
    }
  }, [msResponse]);
  ...
  const signInWithApple = async () => {
    try {
      ...
      signInWithCredential(auth, firebaseCredential).then(u => console.log('Apple user:', u.user));
    } catch (e) {
      console.log(e);
    }
  };
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', gap: 15 }}>
      ...
      <AppleAuthentication.AppleAuthenticationButton buttonType={AppleAuthentication.AppleAuthenticationButtonType.SIGN_IN} buttonStyle={AppleAuthentication.AppleAuthenticationButtonStyle.BLACK} cornerRadius={5} style={{ width: 200, height: 44 }} onPress={signInWithApple} />
    </View>
  );
}
